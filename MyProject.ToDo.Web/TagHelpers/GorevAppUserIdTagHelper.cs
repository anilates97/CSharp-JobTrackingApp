...
namespace MyProject.ToDo.Web.TagHelpers
{
    ...
    public class GorevAppUserIdTagHelper : TagHelper
    {
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            string htmlString = $"<strong> Tamamladığı görev sayısı : </strong>{tamamlananGorevSayisi} " +
                $"<br> <strong>Üstünde çalıştığı görev sayısı : </strong>{ustundeCalistigiGorevSayisi}";

            output.Content.SetHtmlContent(htmlString);  // html string çıktısını ilgili output nesnesine verdik
        }
    }
}
